...
const mocks = vi.hoisted(() => ({
  account: { isConnected: false, address: undefined as `0x${string}` | undefined, chainId: undefined as number | undefined },
...
vi.mock("../runtime", () => ({
  config: { factoryAddress: "0x0000000000000000000000000000000000000001", explorerUrl: "https://etherscan.io" },
  publicClient: { simulateContract: mocks.simulateContract, waitForTransactionReceipt: mocks.waitForTransactionReceipt, readContract: mocks.readContract },
}));
vi.mock("../wallet/WalletControls", () => ({ WalletControls: () => <button>Conectar wallet</button> }));

function renderPage() {
  return render(<QueryClientProvider client={new QueryClient()}><CreateEscrowPage /></QueryClientProvider>);
}
...
async function completeDraft() {
  ...
  await user.type(screen.getByLabelText("Dirección del worker"), "0x0000000000000000000000000000000000000002");
  await user.type(screen.getByLabelText("Dirección del árbitro"), "0x0000000000000000000000000000000000000003");
...
async function reviewConnectedDraft() {
  mocks.account = { isConnected: true, address: "0x0000000000000000000000000000000000000001", chainId: 11155111 };
  renderPage();
...
describe("CreateEscrowPage", () => {
  ...
  it("shows a translated simulation failure before requesting a wallet signature", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent("Cada duración debe ser mayor a cero.");
  ...
  it("retains the hash when confirmation lacks a factory creation event", async () => {
    ...
    const user = await reviewConnectedDraft();
    await user.click(screen.getByRole("button", { name: "Simular y firmar" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("no pudimos identificar el escrow creado");
    expect(screen.getByRole("link", { name: "Ver transacción confirmada" })).toHaveAttribute("href", expect.stringContaining("/tx/0xaaaa"));
  });
});
